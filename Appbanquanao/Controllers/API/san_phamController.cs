...
using Appbanquanao.Models;

namespace Appbanquanao.Controllers.API
{
    public class san_phamController : ApiController
    {
        private DA4_AppbanquanaoEntities1 db = new DA4_AppbanquanaoEntities1();
        ...
        {
            var chon = db.san_pham.Where(x => x.giatien >= minPrice && x.giatien <= maxPrice);
            if (!chon.Any())
            {
                return NotFound();
            }
