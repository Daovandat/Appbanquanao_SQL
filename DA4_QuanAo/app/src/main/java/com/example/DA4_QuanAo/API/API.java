package com.example.DA4_QuanAo.API;
...
public interface API {
    ...
    API API = new Retrofit.Builder().baseUrl("https://lostredbike59.conveyor.cloud/")
    ...
    @GET("api/tk/kiemtra/{taikhoan}/{mk}")
    ...
    @GET("api/ThongKeTaiKhoanTheoSoLuong/{Account}")
    Call<String> Get_ThongKe_TaiKhoan_SoLuong(@Path("Account") int Account);
    @GET("api/chon/TimKiemTheoKhoangGia/{minPrice}/{maxPrice}")
    Call<List<san_pham>> Get_timkiem_khoanggia(@Path("minPrice") int minPrice, @Path("maxPrice") int maxPrice);
}
